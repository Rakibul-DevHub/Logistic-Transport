  ...
  Future<void> _showImageSourceDialog() async {
    ...
    await showModalBottomSheet(
      ...
      if (mounted) {
        ...
        // // If dialog was dismissed without selecting an option, go back
        // if (!_isLoading) {
        //   Navigator.pop(context);
        // }
      }
  ...
  Future<void> _captureImage(ImageSource source) async {
    ...
    try {
      ...
      if (photo != null && mounted) {
        ...
        developer.log('⚠️ User cancelled selection');
        // Go back to previous screen if no image selected
        Navigator.pop(context);
      }
    } catch (e, stack) {
      developer.log('❌ Error selecting image: $e', error: e, stackTrace: stack);
...
class _OCRLoadingScreenState extends State<OCRLoadingScreen> {
  ...
  // Enhanced back button handler with confirmation dialog
  Future<bool> _onWillPop() async {
      ...
      final shouldPop = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                // Reset cubit state before navigating back
                context.read<ScanBolCubit>().resetState();
    ...
    // If not processing, allow back navigation
    return true;
  }
