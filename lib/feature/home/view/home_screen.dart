...
// ---------------------------------------------------------------------------
// Status Overview Section - Updated to show Pending (using draft count)
// ---------------------------------------------------------------------------
    ...
    return BlocBuilder<HomeReportCubit, HomeReportState>(
      builder: (context, state) {
        ...
        final items = <_StatusData>[
          ...
          _StatusData(
            title: 'Pending',
            count: loading ? '--' : _padCount(counts.draft),
            icon: 'assets/icons/pending.svg', // Use draft count for pending
            color: const Color(0xFF6B7280), // Gray color for pending
          ),
          _StatusData(
            title: 'Completed',
...
// ---------------------------------------------------------------------------
// Status Overview Section - Using your existing SVG icons
// ---------------------------------------------------------------------------
...
class _StatusOverviewSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeReportCubit, HomeReportState>(
      builder: (context, state) {
        // Debug to check state
        debugPrint('📊 Status Overview State: ${state.runtimeType}');

        if (state is HomeReportSuccess) {
          final counts = state.data.loadStatusCounts;
          debugPrint('📊 Data - Missing POD: ${counts.missingPod}, Pending: ${counts.pending}, Completed: ${counts.delivered}');
        }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        context.read<HomeReportCubit>().fetch();
                      },
                      child: const Text('Retry'),
                    ),
                  ],
        ...
        final items = <_StatusData>[
          _StatusData(
            title: 'Missing POD',
            count: loading ? '--' : _padCount(counts.missingPod),
            icon: 'assets/icons/missing_pod.svg', // ✅ Your existing icon
            color: const Color(0xFFEAAA08), // Yellow
          ),
          _StatusData(
            title: 'Pending',
            count: loading ? '--' : _padCount(counts.pending),
            icon: 'assets/icons/expense.svg', // ✅ Your existing icon
            color: const Color(0xFF6B7280), // Gray
          ),
          _StatusData(
            title: 'Completed',
            count: loading ? '--' : _padCount(counts.delivered),
            icon: 'assets/icons/completed.svg', // ✅ Your existing icon
