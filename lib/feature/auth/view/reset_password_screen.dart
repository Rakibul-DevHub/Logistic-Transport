...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  // ✅ Updated password rules: 6-8 characters, at least one uppercase
  // letter, one number, and one special character — matches the rules
  // used on Create Account and Account Settings screens.
  String? _validatePassword(String value) {
    if (value.isEmpty) return 'Please create a password';
    if (value.length < 6) return 'Password must be at least 6 characters';
    if (value.length > 8) return 'Password must be less than 8 characters';
