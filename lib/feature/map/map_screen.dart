  ...
  Future<void> _initMapFlow() async {
    ...
    switch (_mode) {
      case MapMode.pickLocation:
        _refreshAddress();
        if (granted) {
          // Always try to center on user first if no initial point
          if (widget.args.initialLat == null || widget.args.initialLng == null) {
  ...
  Future<bool> _ensureLocationPermission({bool requestIfNeeded = true}) async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      if (mounted && requestIfNeeded) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Allow location permission to show your position on map'),
          ),
        );
      }
      return false;
  ...
  void _setupRoute() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        await _mapController?.animateCamera(
          CameraUpdate.newLatLngZoom(pickupLatLng, 10),
        );
      }
    });
  }
      ...
      body: !_permissionChecked
        ...
        children: [
          GoogleMap(
            ...
            // ✅ Show blue my-location mark after permission
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _setupRoute() async {
    ...
    try {
      ...
      await _placesService.getDrivingRoute(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Could not load road route: $error',
          ),
      ...
      body: !_permissionChecked
        ...
        children: [
          GoogleMap(
            ...
            myLocationButtonEnabled: false,
            zoomControlsEnabled: false,
            markers: _mode == MapMode.pickLocation
                ? {}
                : _markers,
            polylines: _polylines,
            mapType: MapType.normal,
          ...
          if (_mode == MapMode.pickLocation)
            _buildPickBottomPanel(),
          if (_mode == MapMode.viewRoute)
            _buildRouteBottomPanel(),
        ],
      ),
    );
  }
          ...
          children: [
            ...
            Text(
              _isMoving
                  ...
                  ? 'Getting address...'
                  : _address ??
                  'Center the pin on your location',
              style: const TextStyle(
                fontSize: 13,
