...
class _MapScreenState extends State<MapScreen> {
  ...
  // --- Map style loaded from asset ---
  String? _mapStyle;  // null until loaded
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(
              Icons.my_location,
              color: _locating ? Colors.grey : AppColors.primaryColor,
            ),
          ),
        ],
      ),
        ...
        children: [
          GoogleMap(
            ...
            onMapCreated: (controller) {
              debugPrint("✅ MapController created – map should be loading tiles.");
          ...
          // Location Loading Text - positioned at top
          if (_locating)
            Positioned(
              top: 16,
              left: 0,
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _loadMapStyle() async {
    try {
      final style = await rootBundle.loadString('assets/map_style/map_style.json');
      if (mounted) {
        setState(() {
          _mapStyle = style;
        });
        debugPrint("✅ Map style loaded successfully (${style.length} chars)");
      }
    } catch (e) {
      debugPrint("⚠️ Failed to load map style: $e");
      // Style remains null – map uses default appearance
    }
  }
  ...
  void initState() {
    ...
    // Load custom map style from asset (doesn't block map creation)
    _loadMapStyle();
        ...
        children: [
          GoogleMap(
            ...
            onMapCreated: (controller) {
              ...
              // Optional: log style error if any
              Future.delayed(const Duration(seconds: 1), () async {
                final error = await controller.getStyleError();
                if (error != null) {
                  debugPrint("⚠️ Map style error: $error");
                }
              });
            },
            onCameraMove: _onCameraMove,
            ...
            polylines: _polylines,
            mapType: MapType.normal,
            style: _mapStyle,   // <-- Applied here; null until loaded
          ),
          // Location Loading Text - positioned at top
          if (_locating)
            Positioned(
