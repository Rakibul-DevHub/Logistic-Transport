...
class _LoadDetailsScreenState extends State<LoadDetailsScreen> {
  ...
  File? _bolImage;
  File? _podImage; // Added for POD image
  String? _bolImageUrl;
  String? _podImageUrl; // Added for POD image URL
  ...
  Future<void> _navigateToPOD() async {
    // Show image source dialog for POD
  ...
  Future<void> _pickPODImage(ImageSource source) async {
    try {
      ...
      if (pickedFile != null) {
        ...
        // Navigate to POD screen with the picked image
        if (mounted) {
          final result = await Navigator.pushNamed(
            context,
            AppRoutes.proofOfDelivery,
            arguments: PODScreenArgs(
              imagePath: _podImage?.path,
              imageUrl: null,
              loadId: _load?.loadId,
              imageFile: _podImage,
            ),
          ...
          // Handle result if POD returns something
          if (result != null && result is String) {
            setState(() {
              _podImage = File(result);
              _podUploaded = true;
            });
          }
        }
...
class _LoadDetailsScreenState extends State<LoadDetailsScreen> {
  ...
  void initState() {
    ...
    if (_load?.podImages != null && _load!.podImages!.isNotEmpty) {
      final podImage = _load!.podImages!.first;
        ...
        child: BlocListener<PODCubit, PODState>(
          listener: (context, state) {
            if (state is PODUploadSuccess) {
              // Update the UI with the uploaded POD image
              setState(() {
                _podImageUrl = state.imageUrl;
                _podUploaded = true;
                _podImage = null; // Clear local file after upload
              });
              ...
              // DO NOT navigate to POD screen - just stay on load details
              // The POD card will automatically show the uploaded image
                        ...
                        children: [
                          ...
                          if (!_podUploaded)
                            _buildWarningCard(
                              ...
                              buttonLabel: 'Upload POD',
                              onUpload: _showImageSourceDialogForPOD,
                              isLoading: context.watch<PODCubit>().state is PODLoading,
                            ),
                          if (!_podUploaded) SizedBox(height: sectionGap),
                          _buildLoadIdCard(),
                          SizedBox(height: sectionGap),
                          ...
                          SizedBox(height: smallGap),
                          BlocBuilder<PODCubit, PODState>(
                            builder: (context, podState) {
                              final isLoading = podState is PODLoading;
                              return CustomElevatedButton(
                                onPressed: isLoading ? null : _showImageSourceDialogForPOD,
                                buttonText: isLoading ? 'Uploading...' : 'Upload POD/Signed BOL',
