...
///
///
/// todo:: internal refresh for status also
///
///
///
///
...
class _LoadDetailsScreenState extends State<LoadDetailsScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Refresh when screen comes back into focus (e.g., from other screens)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _refreshLoadDataIfNeeded();
    });
  }
  ...
  // Check if status has changed and refresh if needed
  void _refreshLoadDataIfNeeded() {
    final currentLoad = _getCurrentLoadData();
    final currentStatus = currentLoad.status;

    // If status changed or it's the initial load
    if (_previousStatus != currentStatus || _isInitialLoad) {
      _isInitialLoad = false;
      _previousStatus = currentStatus;

      final loadId = currentLoad.id?.trim() ?? '';
      if (loadId.isNotEmpty) {
        debugPrint('🔄 Status changed from $_previousStatus to $currentStatus - Refreshing...');
        _loadDetailsCubit.refreshLoadDetails(loadId);
      }
    }
  }
            ...
            listener: (context, state) {
              if (state is LoadDetailsSuccess) {
                ...
                setState(() {
                  ...
                  _previousStatus = state.loadData.status;
                });
