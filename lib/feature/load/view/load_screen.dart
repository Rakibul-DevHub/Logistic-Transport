...
// ✅ Make the state class public so it can be used as a type
class LoadScreenState extends State<LoadScreen> with WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    ...
    // Refresh when screen comes back into focus (after returning from details, etc.)
    if (!_isInitialLoad) {
  ...
  /// Public method to refresh loads from outside (called by BottomNav)
  void refreshLoads() {
    _refreshData();
  }
...
///
///
/// todo:: tryig for inter hidden data load
///
///
///
...
class LoadScreenState extends State<LoadScreen> with WidgetsBindingObserver {
  ...
  /// Public method to refresh loads from outside (called by BottomNav)
  /// ✅ Uses silent refresh - no loading indicator
  void refreshLoads() {
    _refreshDataSilently();
  }

  /// ✅ Silent refresh - no loading indicator shown
  void _refreshDataSilently() {
    if (mounted) {
      _loadListCubit.fetchLoads(
        type: _listType,
        refresh: false, // ✅ false = no loading indicator
      );
    }
  }

  /// ✅ Regular refresh with loading indicator (for pull-to-refresh)
  void _refreshDataWithLoading() {
    if (mounted) {
      _loadListCubit.fetchLoads(
        type: _listType,
        refresh: true, // ✅ true = shows loading indicator
      );
    }
  }
                  ...
                  builder: (context, state) {
                    ...
                    return RefreshIndicator(
                      ...
                      onRefresh: () async {
                        ...
                        await _loadListCubit.fetchLoads(
                          type: _listType,
                          refresh: true,
                        );
                      },
...
// // ✅ EXPORT the LoadScreenState so it can be used as a type in other files
// export 'load_screen.dart' show LoadScreenState;